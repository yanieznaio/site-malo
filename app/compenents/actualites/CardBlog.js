...
const CardBlog = ({ title, description, id, img, showModal, setShowModal }) => {
    ...
    return (
        <motion.div style={{ backgroundImage: `url(${img})` }} onMouseOver={() => setOver(true)} onMouseLeave={() => setOver(false)} variants={variants} whileHover="animate" initial="initial" transition={{ duration: 1, type: 'ease' }} className={`cursor-pointer relative  h-[300px] md:h-[500px]   bg-cover bg-center w-full md:w-1/2
