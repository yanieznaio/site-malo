...
const CardBlog = ({ title, description, id, img, showModal, setShowModal }) => {
    ...
    return (
        <>
            <motion.div onMouseOver={() => setOver(true)} onMouseLeave={() => setOver(false)} whileHover={{ width: "60%" }} initial={{ width: "50%" }} transition={{ duration: 1, type: 'ease' }} className={`hidden md:block cursor-pointer relative  h-[45vh] md:h-[500px]   w-full md:w-1/2
        ...
        `}>
            ...
            <motion.div onMouseOver={() => setOver(true)} onMouseLeave={() => setOver(false)} className={`block md:hidden cursor-pointer relative  h-[45vh] md:h-[500px] w-full `}>
                <div style={{ backgroundImage: `url(${img})` }} className='w-full h-full bg-center bg-cover brightness-75 '>
