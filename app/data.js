

export const services = [
    {
        id: "1",
        title: "Conformité re2020",
        description:" Garantir votre projet de construction et assurer sa réussite en conformité avec la RE2020.",
        list: ["Dossier à envoyer par mail ou courrier", "Réalisation de l'étude thermique RE2020", "Calcul du Bbio", "calcul du Dh", "Création de l'attestation thermique RE2022", "Calcul du Cep", "Calcul du Cepnr", "Calcul ACV", "Calcul du Ic energie", "Calcul du Ic construction", "Création et envoi des calculs attestation fin de travaux" ]
    },
    {
        id: "2",
        title: "Conception bas carbone",
        description:"Le secteur de la construction prend ces dernières années un nouveau virage, pour répondre à un enjeu triple : baisser les consommations d’énergie des bâtiments, réduire les coûts liés à ces dernières et réduire l’empreinte carbone des bâtiments. En France, le bâtiment est en effet à l’origine de 23 % des émissions de gaz à effet de serre... Soit l’un des secteurs les plus émissifs du territoire Vous avez un projet de construction mais vous ne voulez pas plomber  votre bilan carbone ? C’est tout à votre honneur.  Coté environnement vous aide à y voir plus clair !",

    },
    {
        id: "3",
        title: "Préconisation des matériaux",
        description:"Face à l’épuisement des ressources mondiales, l’économie circulaire est un des leviers pour réduire notre impact sur l’environnement. Dans le secteur de la construction, les enjeux sont décisifs puisqu’à lui seul, ce secteur génère 70% des déchets produits en France [ADEME]. La majorité des ressources consommées par le secteur de la construction ne sont ni locales, ni biosourcées et sont majoritairement issues des filières pétrolières ou minières. Notre objectif est de favoriser la mise en oeuvre de matériaux bio ou géo sourcés.",

    },

    {
        id: "4",
        title: "Études thermiques QEB",
        description:" (conseil, études, calculs, contrôle réalisation)",
        list : [
            "Stratégie constructive en concours",
            "Définition de l’enveloppe thermique",
            "Analyse de Cycle de Vie",
            "Simulation Thermique Dynamique",
            " Facteur de Lumière du Jour",
            " Analyse en Coût Global",
            "Écoulement aéraulique interne et externe au bâtiment",
            "Analyse de la Qualité de l’Air Intérieur et/ou Extérieur",
            " Analyse critique des équipements de génie climatique",
            " Coûts et modalités d’entretien-maintenance"
        ]
    },

]

export const prestations = [
    {
        id: 1,
        href: "bas-carbone",
        title: "Conception bas carbone",
        color: "#6096B4",
        description: "ACV, Reemploi, économie circulaire",
        img: '/services/conceptionbascarbone-mobile.webp',
        secondImage: "/services/conception-bas-carbone2.webp",
        resume: "Le secteur de la construction prend ces dernières années un nouveau virage, pour répondre à un enjeu triple : baisser les consommations d’énergie des bâtiments, réduire les coûts liés à ces dernières et réduire l’empreinte carbone des bâtiments. En France, le bâtiment est en effet à l’origine de 23 % des émissions de gaz à effet de serre... Soit l’un des secteurs les plus émissifs du territoire Vous avez un projet de construction mais vous ne voulez pas plomber  votre bilan carbone ? C’est tout à votre honneur.  Coté environnement vous aide à y voir plus clair !",
        info: {
            color: "#F6F5F2",

            img: "",
            title: "Ce qu'il faut savoir",
          
        },
        secondInfo: {


            img: "/nature.webp",
            title: "",
            subtitle: "Construction bas carbonne, une approche à adopter de toute urgence",
            texte: "La construction bas carbone, visant à réduire l'empreinte carbone des bâtiments, est donc l’approche à adopter de toute urgence pour vos projets de neuf. Combinant l'utilisation de matériaux écologiques, le principe d’efficacité énergétique et les énergies renouvelables, elle repense le bâtiment en limitant l’impact environnemental de celui-ci.",


        },
    },
    {
        id: 2,
        href: "acv-re2020",
        title: "Conformité re2020",
        color: "green",
        description: "votre projet conforme RE2020",
        img: "/services/conformite-re2020-mobile.webp",
        secondImage: "/services/conformite-re2020-2.webp",
        white: true,
        resume: "La RE2020 est une réglementation essentielle pour toute nouvelle construction, apportant des bénéfices significatifs en termes d'économie d'énergie, de confort et de valorisation de votre bien. Pour garantir que votre projet de construction respecte cette norme, il est crucial de faire appel à des experts en études thermiques. En adoptant les pratiques recommandées, vous pouvez être sûr de construire une maison durable, performante et conforme aux exigences environnementales actuelles. N'hésitez pas à nous contacter pour vous accompagner dans votre projet et assurer sa réussite en conformité avec la RE2020. ",
        list: ["Dossier à envoyer par mail ou courrier", "Réalisation de l'étude thermique RE2020", "Calcul du Bbio", "calcul du Dh", "Création de l'attestation thermique RE2022", "Calcul du Cep", "Calcul du Cepnr", "Calcul ACV", "Calcul du Ic energie", "Calcul du Ic construction", "Création et envoi des calculs attestation fin de travaux" ],
        info: {
            color: "#F6F5F2",
            whiteText: true,
            img: "",
            title: "Re2020, ce qu'il faut savoir",
           
        },
        secondInfo: {

            img: "/nature.webp",
            title: "",
            subtitle: "Quelles sont les sanctions en cas de non respect de la réglementation environnementale 2020",
            texte: "Il est tres important de comprendre que Des sanctions vous seront , pour eviter cela faites imperativement ces deux etudes , La réglementation environnementale RE2020 doit être parfaitement appliquée par le maître d’oeuvre. **Le non-respect des règles de construction représente un délit passible de sanctions pénales**. Ces dernières peuvent atteindre la somme de 45 000€, qui peut être complétée de 6 mois d’emprisonnement en cas de récidive.",


        },
    },

    {
        id: 3,
        href: "reemplois-economie",
        title: "Préconisation des matériaux",
        color: "blue",
        description: " Vers de nouvelles pratiques dans la construction ",
        img: "/services/preconisation-des-materiaux-mobile.webp",
        secondImage: "/services/bureau-etude-batiments.webp",
        white: true,
        resume: "Face à l’épuisement des ressources mondiales, l’économie circulaire est un des leviers pour réduire notre impact sur l’environnement. Dans le secteur de la construction, les enjeux sont décisifs puisqu’à lui seul, ce secteur génère 70% des déchets produits en France [ADEME]. La majorité des ressources consommées par le secteur de la construction ne sont ni locales, ni biosourcées et sont majoritairement issues des filières pétrolières ou minières. Notre objectif est de favoriser la mise en oeuvre de matériaux bio ou géo sourcés. ",
        info: {
            color: "#F6F5F2",
            whiteText: true,
            img: "",
            title: "",
            faq: [
                {
                    question: "Qu'est-ce que le réemploi dans le secteur du bâtiment ?",
                    answer: (
                        <span>
                            Le <strong>réemploi</strong> dans le secteur du bâtiment consiste à utiliser des matériaux ou des éléments de construction récupérés d'anciens bâtiments dans de nouveaux projets, sans transformation majeure. Cela permet de réduire les déchets et de préserver les ressources naturelles.
                        </span>
                    )
                },
                {
                    question: "Quels sont les avantages du réemploi des matériaux de construction ?",
                    answer: (
                        <span>
                            Les avantages du réemploi des matériaux de construction incluent la réduction des déchets de chantier, la diminution des émissions de CO2, la conservation des ressources naturelles, et souvent des économies financières sur les matériaux.
                        </span>
                    )
                },
                {
                    question: "Comment se déroule la déconstruction sélective ?",
                    answer: (
                        <span>
                            La déconstruction sélective implique de démonter soigneusement un bâtiment afin de récupérer les matériaux réutilisables, tels que les portes, fenêtres, poutres, et briques, qui peuvent ensuite être intégrés dans de nouveaux projets de construction.
                        </span>
                    )
                },
                {
                    question: "Qu'est-ce que l'économie circulaire dans le bâtiment ?",
                    answer: (
                        <span>
                            L'économie circulaire dans le bâtiment est un modèle qui vise à maintenir les matériaux et produits en circulation aussi longtemps que possible. Cela inclut la réutilisation, la réparation, le recyclage et le réemploi des matériaux de construction.
                        </span>
                    )
                },
                {
                    question: "Quels sont les principaux défis du réemploi et de l'économie circulaire dans le bâtiment ?",
                    answer: (
                        <span>
                            Les principaux défis incluent les coûts et la logistique de collecte et de stockage des matériaux réemployables, ainsi que les contraintes réglementaires et normatives qui peuvent limiter l'utilisation de matériaux réutilisés.
                        </span>
                    )
                },
                {
                    question: "Comment la conception pour le démontage contribue-t-elle à l'économie circulaire ?",
                    answer: (
                        <span>
                            La conception pour le démontage consiste à concevoir des bâtiments de manière à ce que leurs éléments puissent être facilement démontés et réutilisés à la fin de leur vie. Cela facilite le réemploi des matériaux et réduit les déchets.
                        </span>
                    )
                },
                {
                    question: "Quelles certifications existent pour encourager l'économie circulaire dans le bâtiment ?",
                    answer: (
                        <span>
                            Des certifications comme <strong>Cradle to Cradle (C2C)</strong> et des labels de bâtiments durables comme <strong>HQE</strong>, <strong>BREEAM</strong>, et <strong>LEED</strong> encouragent les pratiques d'économie circulaire en évaluant et récompensant les projets qui intègrent ces principes.
                        </span>
                    )
                },
                {
                    question: "Quels sont les exemples concrets de projets intégrant le réemploi et l'économie circulaire ?",
                    answer: (
                        <span>
                            Des projets comme le <strong>Pavillon Circulaire à Paris</strong> et la rénovation du siège de <strong>Triodos Bank aux Pays-Bas</strong> sont des exemples concrets où des matériaux réemployés ont été utilisés, démontrant ainsi les avantages et la faisabilité de ces pratiques.
                        </span>
                    )
                },
                {
                    question: "Comment les politiques et régulations soutiennent-elles le réemploi dans le bâtiment ?",
                    answer: (
                        <span>
                            Des politiques spécifiques, comme la <strong>loi anti-gaspillage pour une économie circulaire (AGEC)</strong> en France, encouragent le réemploi et l'économie circulaire en imposant des objectifs de réduction des déchets et en favorisant l'utilisation de matériaux recyclés.
                        </span>
                    )
                },
                {
                    question: "Quel est le rôle d'un bureau d'étude thermique dans la promotion de l'économie circulaire ?",
                    answer: (
                        <span>
                            Un bureau d'étude thermique joue un rôle clé en intégrant des solutions de réemploi et d'économie circulaire dans les projets de construction et de rénovation, en optimisant l'efficacité énergétique et en réduisant l'empreinte carbone globale des bâtiments.
                        </span>
                    )
                }
            ]


        },
        secondInfo: {

            img: "/nature.webp",
            title: "",
            subtitle: "Envisager différemment le cycle de vie des matériaux",
            texte: "Parmi les causes du dérèglement climatique, on trouve la surconsommation des ressources et un gaspillage presque institutionnalisé. L’économie circulaire se propose de revoir en profondeur nos modes de fonctionnement afin de parvenir à une gestion sobre et efficace de nos ressources. ",


        },
    },
    {
        id: 4,
        href: 'etudes-thermiques',
        title: "Étude thermique QEB",
        color: 'blue',
        description: "Lorem ipsum dolor sit amet consectetur.",
        img: "/services/etude-thermique-mobile.webp",
        secondImage: "/contact2.jpg",
        white: true,
        resume: "Face à l’épuisement des ressources mondiales, l’économie circulaire est un des leviers pour réduire notre impact sur l’environnement. Dans le secteur de la construction, les enjeux sont décisifs puisqu’à lui seul, ce secteur génère 70% des déchets produits en France [ADEME]. La majorité des ressources consommées par le secteur de la construction ne sont ni locales, ni biosourcées et sont majoritairement issues des filières pétrolières ou minières. Notre objectif est de favoriser la mise en oeuvre de matériaux bio ou géo sourcés.",
        list : [
            "Stratégie constructive en concours",
            "Définition de l’enveloppe thermique",
            "Analyse de Cycle de Vie",
            "Simulation Thermique Dynamique",
            " Facteur de Lumière du Jour",
            " Analyse en Coût Global",
            "Écoulement aéraulique interne et externe au bâtiment",
            "Analyse de la Qualité de l’Air Intérieur et/ou Extérieur",
            " Analyse critique des équipements de génie climatique",
            " Coûts et modalités d’entretien-maintenance"
        ],
        info: {
            color: "#F6F5F2",
            whiteText: true,
            img: "",
            title: "title",
            faq: [
                {

                    question: "title",
                    answer: (<span>reponse</span>),

                },
                {

                    question: "title",
                    answer: (<span>reponse</span>),

                },

                {

                    question: "title",
                    answer: (<span>reponse</span>),

                },
            ]
        },
        secondInfo: {

            img: "/nature.webp",
            title: "",
            subtitle: "Quelles sont les sanctions en cas de non respect de la réglementation environnementale 2020",
            texte: "Il est tres important de comprendre que Des sanctions vous seront , pour eviter cela faites imperativement ces deux etudes , La réglementation environnementale RE2020 doit être parfaitement appliquée par le maître d’oeuvre. **Le non-respect des règles de construction représente un délit passible de sanctions pénales**. Ces dernières peuvent atteindre la somme de 45 000€, qui peut être complétée de 6 mois d’emprisonnement en cas de récidive.",




        },
    },


]

export const clients = [
    {
        id: 1,
        title: "building",
        description: "Lorem ipsum dolor sit amet consectetur. Aliquam id luctus convallis penatibus. Non laoreet tellus sit accumsan suspendisse.",
        img: '/building.png'
    },
    {
        id: 2,
        title: "Maison individuelle",
        description: "Lorem ipsum dolor sit amet consectetur. Aliquam id luctus convallis penatibus. Non laoreet tellus sit accumsan suspendisse.",
        img: '/house.png'
    },
    {
        id: 3,
        title: "Bureau",
        description: "Lorem ipsum dolor sit amet consectetur. Aliquam id luctus convallis penatibus. Non laoreet tellus sit accumsan suspendisse.",
        img: '/office-building.png'
    },
]

